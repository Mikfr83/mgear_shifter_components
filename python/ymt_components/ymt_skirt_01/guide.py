...
class componentSettings(MayaQWidgetDockableMixin, guide.componentMainSettings):
    ...
    def update_ring_preview(self) -> None:
        ...
        for side in ("L", "R"):
            ...
            for label, center, leg_axis in stations:
                self._create_ring_preview_circle(side, label, center, leg_axis, radius_x, radius_z)
    ...
    def _create_ring_preview_circle(
        ...
        if leg_axis.length() < 0.001:
            pm.displayWarning("ymt_skirt_01 ring preview skipped a degenerate %s_%s leg axis." % (label, side))
            return
        normal = leg_axis.normal()
        reference = datatypes.Vector(0.0, 1.0, 0.0)
        if abs(normal * reference) > 0.99:
            reference = datatypes.Vector(1.0, 0.0, 0.0)
        u_axis = (normal ^ reference).normal()
    ...
    def rebuild_grid_locators(self) -> None:
        ...
        try:
            ...
            axis = self._normalized(((heel_left + heel_right) * 0.5) - waist, "waist-to-heel axis")
            front = self._root_front()
            projected_front = front - (axis * (front * axis))
            projected_front = self._normalized(projected_front, "projected root front")
        ...
        axis_length = (((heel_left + heel_right) * 0.5) - waist).length()
    ...
    def _normalized(self, value: VectorLike, label: str) -> VectorLike:
